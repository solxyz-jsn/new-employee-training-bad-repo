...
import jp.co.solxyz.jsn.springbootadvincedexam.app.admin.user.json.UserId;
...
class UserManagementApiControllerTest {
    ...
    void shouldDeleteUserSuccessfully() {
        ...
        ResponseEntity<List<ObjectError>> response = null;
        try {
            response = controller.deleteUser(userId, bindingResult);
        } catch (Exception e) {
            e.printStackTrace();
            fail();
        }

        verify(service, times(1)).deleteUser(userId.getUserId());
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
    }

    @Test
    @DisplayName("ユーザー情報の削除時に1件バリデーションエラーが発生する場合、1件のエラーメッセージが返却される")
    void shouldFailToDeleteUserDueToValidationError() {
        ...
        verify(service, times(0)).deleteUser(expectedUserId.getUserId());
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(response.getBody().get(0).getDefaultMessage()).isEqualTo("メールアドレスが不正です。");
    }

    @Test
    @DisplayName("ユーザ情報の削除時に2件バリデーションエラーが発生する場合、2件のエラーメッセージが返却される")
    void shouldFailToDeleteUserDueToMultipleValidationErrors() {
        UserId expectedUserId = new UserId();
        expectedUserId.setUserId("123e4567-e89b-12d3-a456-426614174000");
    ...
    void shouldFailToDeleteUserDueToNoSuchElementException() {
        ...
        when(bindingResult.hasErrors()).thenReturn(false);
        doThrow(new NoSuchElementException("ユーザが見つかりません。")).when(service).deleteUser(userId.getUserId());
    ...
    void shouldFailToDeleteUserDueToIllegalStateException() {
        ...
        when(bindingResult.hasErrors()).thenReturn(false);
        doThrow(new IllegalStateException("削除に失敗しました。")).when(service).deleteUser(userId.getUserId());
    ...
    void shouldFailToDeleteUserDueToDataAccessException() {
        ...
        when(bindingResult.hasErrors()).thenReturn(false);
        doThrow(DataAccessResourceFailureException.class).when(service).deleteUser(userId.getUserId());
