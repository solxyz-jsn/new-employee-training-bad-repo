...
class BookReturnServiceTest {
    ...
    void shouldReturnEmptyListWhenCurrentUserBooksIsEmpty() {
        ...
        when(bookRentalBizlogic.getUnreturnedBooksByUserId(userId)).thenReturn(List.of());

        List<UnreturnedBookModel> result = bookReturnService.getCurrentUserBooks(expectedUserId);
    ...
    void shouldReturnCurrentUserBooksSuccessfully() {
        ...
        when(bookRentalBizlogic.getUnreturnedBooksByUserId(userId)).thenReturn(unreturnedBookList);
    ...
    void shouldReturnBookSuccessfully() {
        ...
        try {
            bookReturnService.returnBook(userId, isbn);
        } catch (Exception e) {
            fail();
        }
        verify(bookRentalBizlogic, times(1)).returnBook(expectedUserId, expectedIsbn);
    }
    ...
    void shouldThrowDataIntegrityViolationExceptionWhenReturnBook() {
        ...
        doThrow(DataIntegrityViolationException.class).when(bookRentalBizlogic).returnBook(userId, isbn);

        assertThrows(DataIntegrityViolationException.class, () -> bookReturnService.returnBook(expectedUserId, expectedIsbn));
    }
    ...
    void shouldThrowNoSuchElementExceptionWhenReturnBook() {
        ...
        doThrow(NoSuchElementException.class).when(bookRentalBizlogic).returnBook(userId, isbn);

        assertThrows(NoSuchElementException.class, () -> bookReturnService.returnBook(expectedUserId, expectedIsbn));
    }
}
